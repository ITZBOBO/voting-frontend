...
const colors = {
  bgApp: "bg-gray-50 dark:bg-[#161616]",
  bgCard: "bg-white dark:bg-[#1c1c1a]",
  bgInput: "bg-white dark:bg-[#161616]",
  border: "border-gray-200 dark:border-[#333333]",
  textPrimary: "text-gray-900 dark:text-white",
  textSecondary: "text-gray-500 dark:text-gray-400",
  primaryHover: "hover:bg-gray-50 dark:hover:bg-[#333333]",
  accentBlue: "text-[#2563eb] dark:text-[#3b82f6]",
};
  ...
  return (
    <div className={`p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between border-b ${colors.border} last:border-0 gap-4`}>
      ...
      <div className="flex items-center gap-3 shrink-0">
        <span className="text-sm text-gray-500 dark:text-gray-300">{checked ? "On" : "Off"}</span>
        <button
          type="button"
          role="switch"
          aria-checked={checked}
          onClick={() => onChange(!checked)}
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-[#1c1c1a] ${checked ? "bg-blue-600" : "bg-gray-300 dark:bg-[#3f3f46]"}`}
        >
...
function SectionCard({ title, description, badge, badgeColor, children, icon: Icon }: {
  ...
  const badgeClasses = {
    ...
    green: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-800", // Pill for active
    ...
    <div className={`${colors.bgCard} rounded-xl border ${colors.border} overflow-hidden mb-6`}>
      <div className={`px-4 sm:px-5 py-4 flex items-center justify-between border-b ${colors.border}`}>
        <div className="flex items-center gap-4">
          {Icon && (
            <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-white/10 flex items-center justify-center shrink-0">
...
function StatBox({ title, value, subtext, icon: Icon, valueColor = "text-gray-900 dark:text-white" }: any) {
  return (
...
function WarningBanner({ message }: { message: string }) {
  return (
    <div className="m-4 sm:m-5 p-4 border rounded-xl bg-red-50 border-red-200 dark:bg-red-500/10 dark:border-red-500/20 flex gap-3">
      <AlertTriangle size={18} className="text-red-500 shrink-0 mt-0.5" />
      <p className="text-[15px] text-red-600 dark:text-red-400 leading-relaxed">{message}</p>
    </div>
  );
}
...
const inputCls = "w-full px-3.5 py-2.5 bg-white dark:bg-[#161616] border border-gray-200 dark:border-[#333333] rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition";
  ...
  return (
    <SectionCard title="Voter Authentication" description="Controls how students log into the voting system.">
      <div className={`p-4 sm:p-5 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 border-b ${colors.border}`}>
        <FormField label="Login Method" hint="Determines what identifier students use to log in">
          <select className={inputCls} value={settings.loginMethod} onChange={e => update({ loginMethod: e.target.value })}>
            <option value="matric" className="bg-white dark:bg-[#1c1c1a]">Matric Number</option>
            <option value="email" className="bg-white dark:bg-[#1c1c1a]">School Email</option>
          </select>
        </FormField>
...
function AdminsTab({ qc }: { qc: any }) {
  ...
  const roleColor: Record<string, string> = {
    ...
    observer: "text-gray-600 border border-gray-400 bg-gray-100 dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800",
  };
  ...
  return (
    <>
      <SectionCard title="Admin Management" description="Manage administrator accounts and their roles.">
        ...
        {showAdd && (
          <div className={`m-4 sm:m-5 p-4 sm:p-5 border ${colors.border} bg-gray-50 dark:bg-[#161616] rounded-xl space-y-4`}>
            <h4 className={`text-sm font-semibold ${colors.textPrimary}`}>New Admin User</h4>
            {formError && <div className="p-2.5 bg-red-100 border border-red-200 text-red-700 dark:bg-red-900/30 dark:border-red-800 dark:text-red-400 text-xs rounded-lg">{formError}</div>}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField label="Role">
                <select className={inputCls} value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}>
                  <option value="super_admin" className="bg-white dark:bg-[#1c1c1a]">Super Admin</option>
                  <option value="admin" className="bg-white dark:bg-[#1c1c1a]">Admin</option>
                  <option value="election_officer" className="bg-white dark:bg-[#1c1c1a]">Election Officer</option>
                  <option value="observer" className="bg-white dark:bg-[#1c1c1a]">Observer</option>
                </select>
              </FormField>
            </div>
            <div className="flex gap-2 pt-2">
              <button onClick={() => { setShowAdd(false); setFormError(""); }} className="px-4 py-2 border border-gray-300 dark:border-[#444] text-gray-600 dark:text-gray-300 rounded-lg text-sm hover:bg-gray-100 dark:hover:bg-[#333] transition">Cancel</button>
        ...
        {isLoading ? (
          ...
          <div className={`px-4 sm:px-5 pb-5 divide-y divide-gray-100 dark:divide-[#333]`}>
            {adminUsers.map(u => (
              <div key={u.id} className={`flex flex-col sm:flex-row sm:items-center justify-between py-4 gap-3`}>
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-full bg-gray-100 dark:bg-[#1c1c1a] border border-gray-200 dark:border-[#333] text-gray-500 dark:text-gray-300 flex items-center justify-center font-bold text-sm shrink-0`}>
                ...
                <div className="flex items-center gap-2 flex-wrap sm:justify-end">
                  {u.roles.map(r => (
                    <span key={r} className={`px-2 py-0.5 rounded-full text-[11px] font-semibold tracking-wide ${roleColor[r] || "text-gray-500 dark:text-gray-400"}`}>
                      {r.replace("_", " ").toUpperCase()}
                    </span>
                  ))}
                  <button
                    onClick={() => toggleMut.mutate({ id: u.id, isActive: !u.isActive })}
                    className={`px-3 py-1 rounded-md text-xs font-medium transition ${u.isActive ? "bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-400 hover:bg-green-200 dark:hover:bg-green-500/20" : "bg-red-100 text-red-600 dark:bg-red-500/10 dark:text-red-500 hover:bg-red-200 dark:hover:bg-red-500/20 border border-transparent"}`}
                  >
          ...
          {departments.map((dept: any) => (
            <div key={dept.id} className="flex items-center justify-between py-3 gap-3">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {editId === dept.id ? (
                  <>
                    <button onClick={() => updateMut.mutate({ id: dept.id, name: editName })} className={`w-8 h-8 flex items-center justify-center text-green-600 dark:text-green-500 ${colors.primaryHover} rounded transition`}><Check size={14} /></button>
                    <button onClick={() => setEditId(null)} className={`w-8 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 ${colors.primaryHover} rounded transition`}><X size={14} /></button>
                  </>
                ) : (
                  <>
                    <button onClick={() => { setEditId(dept.id); setEditName(dept.name); }} className={`w-8 h-8 flex items-center justify-center text-blue-600 dark:text-blue-500 ${colors.primaryHover} rounded transition`}><Edit2 size={14} /></button>
                    <button onClick={() => deleteMut.mutate(dept.id)} className={`w-8 h-8 flex items-center justify-center text-red-600 dark:text-red-500 ${colors.primaryHover} rounded transition`}><Trash2 size={14} /></button>
                  </>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {(["csv","json"] as FmtType[]).map(f => (
            <button key={f} onClick={() => setFmt(f)}
              className={`px-4 py-1.5 rounded-lg text-sm font-medium border transition ${fmt === f ? "bg-blue-600 text-white border-blue-600" : `bg-transparent text-gray-500 dark:text-gray-400 border-gray-300 dark:border-[#444] ${colors.primaryHover}`}`}>
              {f.toUpperCase()}
            </button>
          ))}
        </div>
      ...
      <div className="p-4 sm:p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <button key={label} onClick={action}
            className={`flex flex-col items-start p-5 rounded-xl border border-gray-200 dark:border-[#333] hover:bg-gray-50 dark:hover:bg-[#333]/50 transition cursor-pointer text-left bg-white dark:bg-[#1c1c1a]`}>
  ...
  return (
    <SectionCard title="Audit Logs" description="Track all major actions performed by admins and voters.">
      <div className={`p-4 sm:p-5 border-b ${colors.border} flex items-center gap-3`}>
        ...
        <button onClick={() => refetch()} disabled={isFetching} className={`flex items-center gap-2 px-3.5 py-2.5 border border-gray-200 dark:border-[#444] rounded-lg text-sm text-gray-500 dark:text-gray-400 ${colors.primaryHover} transition`}>
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-[#1c1c1a] border-b border-gray-200 dark:border-[#333]">
                <th className="py-3 px-4 sm:px-5 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">Action</th>
                <th className="py-3 px-4 sm:px-5 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">Actor</th>
                <th className="py-3 px-4 sm:px-5 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">Time</th>
              </tr>
            </thead>
            <tbody>
              {filtered.slice(0, 50).map((log: any) => (
                <tr key={log.id} className="border-b border-gray-100 dark:border-[#333] hover:bg-gray-50 dark:hover:bg-[#333]/30 transition last:border-0">
                  <td className="py-3 px-4 sm:px-5">
                    <span className={`px-2.5 py-1 rounded-md text-[11px] font-semibold text-gray-700 bg-gray-200 dark:text-gray-300 dark:bg-[#333]`}>
